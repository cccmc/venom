...
class variant_t {
   public:
    ...
    variant_t(const char* var) {
        set_defaults();
        set(std::string(var));
    }
    void reset() {
        m_type = vartype_t::TYPE_UNUSED;
    }
    ...
    const vector2_t& get_vector2() const {
        return *((vector2_t*)m_var);
    }
    const vector3_t& get_vector3() const {
        return *((vector3_t*)m_var);
    }
    ...
    std::string print() {
        switch (get_type()) {
            ...
            case vartype_t::TYPE_VECTOR2: return "x: " + std::to_string(get_vector2().x) + " y: " + std::to_string(get_vector2().y);
            case vartype_t::TYPE_VECTOR3:
                return "x: " + std::to_string(get_vector3().x) + " y: " + std::to_string(get_vector3().y) + " z: " + std::to_string(get_vector3().z);
   ...
   public:
    ...
    int getsize()
    {
        for (auto i = 0; i < INT32_MAX; ++i) {
            if (get(i).get_type() == variant_t::vartype_t::TYPE_UNUSED)
                return i - 1;
        }
        return 0;
    }

    std::string get2(int num)
    {
        std::string ret;

        auto& variant = m_variant[num];
        switch (variant.get_type()) {
        case variant_t::vartype_t::TYPE_UNUSED:
        case variant_t::vartype_t::TYPE_ENTITY:
        case variant_t::vartype_t::TYPE_COMPONENT:
        {
            break;
        }
        case variant_t::vartype_t::TYPE_FLOAT: {
            ret = std::to_string(variant.get_float());
            break;
        }
        case variant_t::vartype_t::TYPE_STRING: {
            ret = variant.get_string();
            break;
        }
        case variant_t::vartype_t::TYPE_VECTOR2: {
            auto& temp = variant.get_vector2();
            ret += "x: ";
            ret += std::to_string(temp.x);
            ret += ", y: ";
            ret += std::to_string(temp.y);
            break;
        }
        case variant_t::vartype_t::TYPE_VECTOR3: {
            auto& temp = variant.get_vector3();
            ret += "x: ";
            ret += std::to_string(temp.x);
            ret += ", y: ";
            ret += std::to_string(temp.y);
            ret += ", z: ";
            ret += std::to_string(temp.z);
            break;
        }
        case variant_t::vartype_t::TYPE_UINT32: {
            ret = std::to_string(variant.get_uint32());
            break;
        }
        case variant_t::vartype_t::TYPE_RECT: {
            auto& rect = variant.get_rect();
            ret += "m_h: ";
            ret += std::to_string(rect.m_h);
            ret += ", m_w: ";
            ret += std::to_string(rect.m_w);
            ret += ", m_x: ";
            ret += std::to_string(rect.m_x);
            ret += ", m_y: ";
            ret += std::to_string(rect.m_y);
            break;
        }
        case variant_t::vartype_t::TYPE_INT32: {
            ret = std::to_string(variant.get_int32());
            break;
        }
        }
        return ret;
    ...
    uint8_t* serialize_to_mem(uint32_t* size, uint8_t* data) const {
        int vars_used = 0;
        int mem_needed = 0;
        int var_size;
        for (int i = 0; i < C_MAX_VARIANT_LIST_PARMS; i++) {
    ...
    std::string print() {
        ...
        for (int i = 0; i < C_MAX_VARIANT_LIST_PARMS; i++) {
            if (m_variant[i].get_type() == variant_t::vartype_t::TYPE_UNUSED)
                continue;
            ss << std::to_string(i) << ": " << m_variant[i].print() + "\n";
        }
        if (ss.str().empty())
            ss.str("(none)");
        return ss.str();
